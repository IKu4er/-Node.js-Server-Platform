...
власні бібліотеки та інші цікаві рішення. Але в основному використовуються можливості і синтаксис всім звичного JavaScript.
  ...
  * **Величезна кількість зовнішніх бібліотек і готових модулів.** Використання пакетного менеджера NPM дозволяє постійно розвивати екосистему Node. Сьогодні число опенсорсних пакетів в ньому перевалило за цифру 500 тисяч і постійно зростає.
...
* Запуск Node.js на x86 Windows повинен працювати, і надавати виконавчі файли. Проте тести в нашій інфраструктурі працюють тільки на WoW64. Більш того, компіляція в x86 Windows є експериментальною і може перестати працювати.
...
## Установка пакетів:
Так як користувачі частіше скачують пакети (16 млрд завантажень проти 13 млн публікацій), добре б було розібратися, як їх встановлювати.
- npm install
...
```
Ці залежності встановлюються командною ```npm install``` з прапорами ```--save і --save-dev```. Вони призначені відповідно для використання в продакшені і розробці.
...
* Файл package.json
Файл ```package.json``` містить в собі інформацію про вашому додатку: назва, версія, залежно тощо. Будь-яка директорія, в якій є цей файл, інтерпретується як Node.js-пакет, навіть якщо ви не збираєтеся публікувати його.
